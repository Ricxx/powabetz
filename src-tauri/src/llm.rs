...
fn user_prompt(
    ...
    let strategy_block = match opts.strategy.as_str() {
        ...
        "legmonster" => "LEG MONSTER mode — MAXIMUM same-game construction. For EACH fixture in the table build ONE giant SGP of UP TO 10 LEGS (aim 8-10; never fewer than 6), using only VIABLE legs (est_prob ≥ ~0.40, no junk fillers): weave one coherent game story — result lean, team goals, BTTS where it fits, corners over, cards where the matchup is physical, the key players' scorer/SOT/shots lines. Multiple fixtures = one 10-leg monster PER fixture, each its own ticket. QUALITY GATE: every leg must independently defend its place; NEVER include nested same-player legs or mutually-implied lines — with 10 legs one sloppy pick kills the ticket. CROSS-CHECK: where ingested page data appears in the notes, verify each leg against it (a page's corner/cards numbers or team news can veto or upgrade a leg — say so in the why). In each 'why': the game story in one sentence + which leg is most fragile.",
        "powerof3" => "POWER OF 3 mode — the compounding cash-out build. FOR EVERY FIXTURE in the table, assemble a SAME-GAME BLOCK of 2-3 VERY LIKELY, LOW-VARIANCE legs (each leg est_prob ≥ ~0.55 and priced ~1.15-1.9 — favourite result/DC, team over 0.5/1.5 goals, a reliable SOT or corners line) whose COMBINED price lands between 1.7x and 4x — aim ~3x per game. Prefer POSITIVELY-REINFORCING legs inside a block (favourite to win + their team goals + their striker involved: one game-script). Then MULTIPLY the blocks across ALL selected fixtures into ONE SGP+ ticket — every fixture contributes a block, no exceptions (4 games ≈ 3x·3x·3x·3x ≈ 81x). Cross-game blocks are INDEPENDENT events, so the compounding is real, not correlation-inflated. Build 4-5 SUCH TICKETS with genuinely different blocks (rotate the block construction per game — result-led, goals-led, props-led). ORDER each ticket's blocks by KICKOFF TIME where the context notes reveal it, SAFEST blocks first — these tickets are built for the cash-out curve: surviving the first two games should leave a strong offer. In each 'why': list each game's block price and the running product (e.g. '2.9x → 8.4x → 26x → 79x').",
...
pub async fn call_model(
    ...
    let mut max_tokens = (3000 + count * per_ticket + opts.lucky_total() as i64 * 560).clamp(4000, 24000);
    // DeepSeek output is ~5x cheaper than Haiku's — never starve it. A bigger
    // floor buys fuller slates and richer 'why's for cents.
...
pub async fn anthropic_call(
    ...
    if deepseek {
        if ds_thinking && max_tokens >= 12_000 {
            // BUILD calls with the Settings toggle ON: DeepSeek reasons about
            // the slate — sharper reads, MUCH slower (user's call, default off).
            body["thinking"] = serde_json::json!({ "type": "enabled", "budget_tokens": (max_tokens / 2).min(12_000) });
        } else {
            // Default / data-extraction: thinking off — fast, answer-only.
