...
BET BUILDERS (REQUIRED): include AT LEAST 3 multi-leg bet builders (SGP — same fixture, or SGP+ — across fixtures) of 3-5 legs each, deliberately chosen so the legs' est_prob values MULTIPLY to roughly a 10-20% combined hit chance (genuine longshots, ~5x-10x). Use each leg's est_prob to gauge this. Within an SGP prefer CORRELATED legs (e.g. a team to win + their key player to score + over 2.5) — real correlated-SGP prices are usually shorter than the naive product of odds.
