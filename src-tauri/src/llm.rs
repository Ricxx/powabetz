...
fn user_prompt(
    ...
    let cover_block = if opts.cover_all {
        " COVERAGE REQUIREMENT: every multi-leg ticket (SGP+/parlay) MUST include AT LEAST ONE leg from EVERY fixture in the table — no match may be left off a cross-game ticket. If a fixture has no defensible leg, say so in data_quality_notes rather than silently skipping it."
    } else {
        ""
    };
